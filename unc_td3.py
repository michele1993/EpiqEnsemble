...
class unc_TD3():
    def __init__(
        ...
        # Initialise two extra (target) critics
        self.critic = critic
        if full_ensemble:
            self.critic_target = CriticEnsemble(d_action, d_state, critic.n_units, critic.n_layers, q_ensemble_s, critic.activation, critic.ln_rate, critic.grad_clip, self.device)

        else:
            self.critic_target = CriticHeadEnsemble(d_action, d_state, critic.n_units, critic.n_standard_layers, q_ensemble_s, critic.activation, critic.ln_rate, critic.grad_clip, self.device, critic.n_ensb_layers) 
    ...
    def update(self, states, actions, rewards, next_states, masks):
       ...
       """
        ...
        if self.step_counter % self.policy_delay == 0:
            ...
            targ_loss = torch.mean(targ_q,dim=0) - torch.std(targ_q,dim=0) #+ 0.1 * torch.mean((det_a - actions.detach())**2,dim=1)
            #targ_loss = torch.mean(targ_q,dim=0) + torch.std(targ_q,dim=0)
            actor_loss = self.actor.update(targ_loss).detach()

            # ------- Optimise behavioural policy -------
            # Compute actor loss
            q = self.critic.forward_all(states, actions)  
            q1, q2 = torch.split(q, self.q_ensemble_s, dim=0) # divide predictions from two targets Q ensembles
            targ_q = torch.min(q1,q2)
            behv_loss = torch.mean(targ_q,dim=0) + torch.std(targ_q,dim=0) #+ 0.1 * torch.mean((det_a.detach() - actions)**2, dim=1)
            q_actor_loss = self.q_actor.update(behv_loss).detach()
